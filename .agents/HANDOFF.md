...
- Known GitHub release state: Public repository `https://github.com/iNGlY/DIT_Renamer` uses `main` as its default branch. Annotated tag `v1.1.0` remains immutable at `11227ce`; public tag `v1.1.1` and Latest Release point to the bilingual universal 1.1.1 ad-hoc assets with DMG, ZIP, SHA-256, LICENSE, and NOTICE. GitHub CLI 2.97.0 is authenticated as `iNGlY`. GitHub Pages serves `main:/docs`; the live signed appcast is available at `https://ingly.github.io/DIT_Renamer/appcast.xml`.
