...
- Known GitHub prerelease state: GitHub CLI 2.97.0 is installed but not authenticated, and no `origin` remote exists. No tag, push, repository, or GitHub Release has been created.

## Next handoff

Next agent may edit and test on this task branch. Do not merge into `main` until the real-card remount acceptance test and remaining field-safety review items are explicitly confirmed. Obtain user approval before removing the speculative `codex-hde` probe or relabeling the HDE result card. To publish the GitHub prerelease, the user must complete `gh auth login` and provide or confirm the target repository `owner/name` and visibility; then configure `origin`, rebuild from final HEAD, and run `scripts/publish_github_release.sh`.
