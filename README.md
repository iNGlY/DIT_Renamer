# DIT Renamer (现场卡卷重命名与自动化助手 - V0.1 开源专业版)

**DIT Renamer** 是一套专为影视 DIT（数字影像技术员）、数据现场工程师及助理开发的高度定制化卡卷管理、智能重命名与安全防护桌面端系统。

本软件结合了真实片场苛刻工况下的无数实战经验（包括克服 APFS 空间管理器损坏、闪迪 4T 掉盘门避坑、与 Pomfort Silverstack 无缝连通），旨在建立一套**零误触、全自动、绝对可溯源**的现场卡卷处理流水线。
...
### 3. 🔄 插卡无感全自动重命名 (V0.1 特色)
- 在左侧侧边栏开启“⚡ 全自动重命名”开关后，系统在后台每秒轮询一次新装载设备。
- 探测到未命名的相机原始卡插入时，自动推断递增卡号（例如自动推断前序为 A001，将当前卡分配为 `A002_0701Z_SONY`），执行底层重命名并重新安全挂载。
- **插卡即拔卡**：如果推断规则出现歧义，软件会自动中止并亮起黄色警示，等待人工介入确认，保障万无一失。

### 4. 🔗 Pomfort Silverstack 联动通信 (端口 8088)
- 内置本地 Webhook HTTP 服务端（监听 `http://localhost:8088/silverstack-webhook`）。
- 当 Silverstack 完成拷贝卸载时，自动接收回调通知并同步显示在软件实时现场日志面板中。

---

## 📚 现场 DIT 必备技术手册库 (`docs/`)

为规范现场作业标准，本项目内置了两套珍贵的实战业务指南（见 `docs/` 目录）：
...
DIT_Renamer/
├── src/                    # 源代码
│   ├── app.py              # PyWebView 桌面端主程序与内置 HTTP 引擎
│   └── web/                # 前端 UI 与控制器
...
```
编译完成后，前往 `~/Downloads/dit_renamer_V0.1.app` 即可双击打开使用！
