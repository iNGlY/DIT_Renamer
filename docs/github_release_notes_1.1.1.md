...
DIT Renamer 1.1.1 是面向 macOS 14+ 的原生 Swift 应用。它在拷贝开始前识别可移除摄影机媒体，提供由操作员确认的卷名建议，并记录每次重命名。本版本加入 Sparkle 应用内更新。
...
- 支持手动选择是否保留检测到的 suffix，并执行 FAT、MS-DOS 与 exFAT 的 11 字符卷名限制。
- 始终排除 Apple Disk Image Media、网络卷和系统虚拟卷。
...
- 启动时检查更新，发现新版本后才显示提示。确认更新后，应用在原安装路径完成替换并重新启动。
...
DIT Renamer 1.1.1 is a native Swift application for macOS 14+. It identifies removable camera media before offload, suggests an operator-confirmed volume name, and records each rename. This release adds forced remount after rename, Sony metadata detection, localized read-only ParaShoot reports, and Sparkle in-app updates.
